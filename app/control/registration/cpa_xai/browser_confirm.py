...
def inject_cookies(page: Any, cookies: Any, log: LogFn | None = None) -> int:
    ...
    if not items or page is None:
        return 0
    # DrissionPage writes explicit-domain cookies through CDP.  Navigating three
    # remote hosts before doing so can each wait on a slow WARP/Cloudflare route,
    # turning a cookie restore into a multi-minute operation.
    ...
    try:
        ...
        if n == 0 and js_items:
            # Only use document.cookie when the CDP bulk APIs were unavailable.
            # The normal CDP path can set httpOnly/domain cookies without navigation.
            page.get("https://accounts.x.ai/", timeout=12)
            for c in js_items:
                name = str(c["name"])
                val = str(c["value"])
    ...
    *,
    ...
    try:
        ...
        if cookies:
            ...
            # inject_cookies() already records the successful method/count.
            try:
                work_page.get("https://accounts.x.ai/", timeout=15)
                _sleep(0.6)
