...
async def serve_video(id: str = Query(..., description="Video file ID")):
    ...
    Async ``/v1/videos`` jobs use IDs such as ``video_<uuid>`` while the
    synchronous/chat-completions local-output path uses a 32-character SHA-1
    ID.  Both are generated by this application and must be accepted here.
    Keep the allow-list strict so the ID cannot escape the video directory.
    """
    import re

    # Do not use Path.resolve() on user input or accept arbitrary filenames:
    # the endpoint always appends ``.mp4`` inside the fixed media directory.
    # The underscore is required for the native async video job ID format.
    if not re.fullmatch(r"(?:video_[0-9a-f]{32}|[0-9a-f-]{16,40})", id):
